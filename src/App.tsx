...
import { MediaItem, UserRole, AppSettings } from './types';
import { Plus, Camera, Settings as SettingsIcon, Lock, Shuffle, Clock, User, Info, CloudOff } from 'lucide-react';
import { fetchPosts, deletePost, createPost, fetchSettings, saveSettings } from './services/supabaseClient';
...
const App: React.FC = () => {
  ...
  const handleUpdateSettings = async (newSettings: AppSettings) => {
    ...
    if (newSettings.aboutHtml !== appSettings.aboutHtml) {
      try {
        ...
        alert("Saved locally, but failed to sync to cloud. Check your connection.");
      }
    }
  };
  ...
  const displayItems = useMemo(() => {
    ...
    if (sortMode === 'RECENT') {
      ...
      items = items.sort(() => Math.sin(seed + Math.random()) - 0.5);
    }
  ...
  const handleUpload = async (newItem: MediaItem) => {
    ...
    try {
      ...
      alert("Upload failed to save to database. Please check your configuration.");
  ...
  const isMaintenanceActive = appSettings.maintenanceMode && userRole !== 'ADMIN';
        ...
        {isMaintenanceActive ? (
          <div className="flex flex-col items-center justify-center h-[60vh] animate-fade-in">
            ...
            <p className="text-zinc-400 max-w-md text-center">
              The gallery is currently undergoing scheduled maintenance. 
          ...
          <>
            ...
            {/* About Section */}
            <AboutSection isOpen={isAboutOpen} htmlContent={appSettings.aboutHtml} />
            ...
            {!isLoadingData && displayItems.length === 0 && (
              <div className="flex flex-col items-center justify-center h-96 text-zinc-500 animate-fade-in">
                ...
                <p className="text-sm max-w-xs text-center mt-2">Try uploading something or check your database connection.</p>
              </div>
            )}
